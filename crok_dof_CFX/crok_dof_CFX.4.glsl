/*
DoF with bokeh GLSL shader v2.4(simplified)
...
float zfar = 10000.0; //camera clipping end
...
float bdepth(vec2 coords) //blurring depth
{
	...
	for( int i=0; i<9; i++ )
	{
		float tmp = texture2D(adsk_results_pass2, coords + offset[i]).r;
		d += tmp * kernel[i];
	}
...
vec3 color(vec2 coords,float blur) //processing the sample
{
	...
	col.r = texture2D(adsk_results_pass3,coords + vec2(0.0,1.0)*texel*fringe*blur).r;
	col.g = texture2D(adsk_results_pass3,coords + vec2(-0.866,-0.5)*texel*fringe*blur).g;
	col.b = texture2D(adsk_results_pass3,coords + vec2(0.866,-0.5)*texel*fringe*blur).b;
...
void main() 
{
	...
	//float depth = linearize(texture2D(adsk_results_pass2,gl_TexCoord[0].xy).x);
	float depth = texture2D(adsk_results_pass2,gl_TexCoord[0].xy).x;
	...
	if (autofocus)
	{
		fDepth = linearize(texture2D(adsk_results_pass2,focus).x);
	}
	...
	{
		col = texture2D(adsk_results_pass3, gl_TexCoord[0].xy).rgb;
	}
