...
float time = adsk_time *.05;
	...
	void main()
	{
			...
			vec2 suncent=vec2(0.6,0.9);
			
			float suns=(1.0-distance(uv,suncent));
			suns=clamp(0.2+suns,0.0,1.0);
			float sunsh=smoothstep(0.15,0.95,suns);

			float slope;
			slope=0.48+uv.x-(uv.y*3.3);
			slope=1.0-smoothstep(0.95,0.0,slope);								
			
			float noise=abs(fbm(uv*1.));
			slope=(noise*0.41)+(slope-((1.0-noise)*slope*0.1))*0.6;
			slope=clamp(slope,0.36,.65);
			...
			Snowout = 0.35+(slope*(suns+0.6))+(sunsh*0.1)+N1+N2+N3+N4+N5;
			
			gl_FragColor = vec4(Snowout*0.84, Snowout * 0.86, Snowout*0.94, 1.0);
