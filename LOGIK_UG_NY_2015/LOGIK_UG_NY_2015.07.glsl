...
vec2 caret_origin = vec2(83.0, 0.11);
...
{ 	
	...
	// CineSys
	d += C(r()); add(); d += I(r()); add(); d += N(r()); add(); d += E(r()); add(); d += S(r()); add(); d+= Y(r()); add(); d += S(r());space(); space();
	
	// Oceana
	d += O(r()); add(); d += C(r()); add(); d += E(r()); add(); d += A(r()); add(); d += N(r()); add(); d += A(r()); add(); space();space();
