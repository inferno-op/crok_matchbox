...
vec2 caret_origin = vec2(23.3, 0.15);
...
{ 	
	...
	vec3 col = texture2D(adsk_results_pass12, uv).rgb;
	...
	// logik
	d += L(r()); add(); d += O(r()); add(); d += G(r()); add(); d += I(r()); add(); d += K(r());space(); space();
	
	// UG
	d += U(r()); add(); d += G(r()); space();space();
	
	// MEETING
	d += M(r()); add(); d += E(r()); add(); d += E(r()); add(); d += T(r()); add(); d += I(r()); add(); d += N(r()); add(); d += G(r()); space(); space();
	
	// Berlin
	d += B(r()); add(); d += E(r()); add(); d += R(r()); add(); d += L(r()); add(); d += I(r()); add(); d += N(r()); space(); space(); space(); space();
	
	// A New Hope
	d += A(r()); add();  space(); d += N(r()); add(); d += E(r()); add(); d += W(r());  space(); add(); d += H(r()); add(); d += O(r()); add(); d += P(r()); add(); d += E(r());
