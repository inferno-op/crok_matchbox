...
vec2 caret_origin = vec2(71.7, 0.11);
...
{ 	
	...
	vec3 col = texture2D(adsk_results_pass13, uv).rgb;
	...
	// CinePostproduction
	d += C(r()); add(); d += I(r()); add(); d += N(r()); add(); d += E(r()); add(); d += P(r()); add(); d+= O(r()); add(); d += S(r()); add(); d += T(r());add(); d += P(r()); add(); d += R(r());add(); d += O(r());add(); d += D(r()); add(); d += U(r()); add(); d += C(r());add(); d += T(r()); add(); d += I(r()); add(); d += O(r()); add(); d += N(r());  space(); space();
	
	// Berlin
	d += B(r()); add(); d += E(r()); add(); d += R(r()); add(); d += L(r()); add(); d += I(r()); add(); d += N(r()); add(); space();space();
