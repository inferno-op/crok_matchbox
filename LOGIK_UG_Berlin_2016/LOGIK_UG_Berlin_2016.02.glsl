...
float pVignettessoftness = 0.9;
float pVignettescale = 0.15;
bool pAddGrain = true;
...
bool moire= true;
bool add_vignette= true;
bool tv_tube_vignette= true;
...
vec2 scandistort(vec2 uv) {
	...
	uv.x -= pDistort * mix(texture2D(adsk_results_pass1, vec2(uv.x, amount)).r * amount, amount, 0.9);
	return uv;
}
...
vec3 getVideo(vec2 uv)
{
	...
	vec3 video = vec3(texture2D(adsk_results_pass1,look));
	return video;
}
...
void main(void)
{
	...
	vec4 color = texture2D(adsk_results_pass1, crt_uv).rgba;
