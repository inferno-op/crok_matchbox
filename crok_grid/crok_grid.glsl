...
float s=texture2D(Source,vec2(.5+pos*.5,.1)).x;
...
void main(void)
{
	...
	mat3 camrot=rotmat(normalize(vec3(1.,0.,0.)),1.)*rotmat(normalize(vec3(1.,0.*sin(.1),0.)),165.+30.*sin(.5));
	float s=.1,maxc=0.;
	vec3 p1,p2,p3;
	vec3 rotv=vec3(0.,0.,1.);
	float h;
	vec3 col=vec3(0.);
	float dist=1.2+pow(abs(sin(.3)),5.)*.5;
	vec3 c=vec3(0.);
