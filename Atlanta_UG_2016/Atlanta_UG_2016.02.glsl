...
vec3 Fetch(vec2 pos,vec2 off){
  ...
  return ToLinear(texture2D(adsk_results_pass1,pos.xy,-16.0).rgb);}
...
void main() 
{
 	...
 	vec3 col = Tri(pos)*Mask4(gl_FragCoord.xy);
	col += Bloom(pos)*bloomAmount;
  
	float blend_anim = smoothstep(0., 10., time - .25);
	float closing = smoothstep(10., 0., time - 100.);
	
	// add letterbox
  	col *= step(abs(pos.y - 0.5 + offset), blend_anim * .3);
  	col *= step(abs(pos.y - 0.5 + offset), closing * .3);
