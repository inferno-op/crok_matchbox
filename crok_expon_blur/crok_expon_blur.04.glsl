...
uniform sampler2D adsk_results_pass3, Strength;
...
void main(void)
{
	...
	if ( proportinal )
		blur = gblur(adsk_results_pass3, b_amount * 0.1 * 4.0, vec2(0.0, 1.0) );
	
	else
	{
		// vertical only
		blur = gblur(adsk_results_pass3, b_xy_amount.x * 0.1 *4.0, vec2(0.0, 1.0) );
	}
