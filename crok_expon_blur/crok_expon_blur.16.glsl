...
uniform sampler2D adsk_results_pass15, Strength;
...
void main(void)
{
	...
	if ( proportinal )
		blur = gblur(adsk_results_pass15, b_amount * 0.1 * 256.0, vec2(0.0, 1.0) );
	
	else
	{
		// vertical only
		blur = gblur(adsk_results_pass15, b_xy_amount.x * 0.1 * 256.0, vec2(0.0, 1.0) );
	}
