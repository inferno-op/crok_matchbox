...
{ 	
	...
	vec3 col = texture2D(adsk_results_pass3, uv).rgb;
	
	float d = 0.;
	
	caret = caret_origin;

	//d += STOP(r()); add(); d += STOP(r()); add(); d += STOP(r());
	
	// The
	d += T(r()); add(); d += H(r()); add(); d += E(r());space(); space();
	
	// Salisbury
	d += S(r()); add(); d += A(r()); add(); d += L(r()); add(); d += I(r()); add(); d += S(r()); add(); d += B(r()); add(); d += U(r()); add(); d += R(r()); add(); d += Y(r()); space();space();
